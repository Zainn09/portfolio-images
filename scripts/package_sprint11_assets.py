#!/usr/bin/env python3
"""Build and integrity-check the downloadable Sprint 11 package.

Sprint 11 covers project IDs 95-99. Four live storefronts produced complete
asset sets; Vintage Art Garage (98) is documented as publicly unavailable and
contributes a documentation folder only.
"""
...
SPRINT = ROOT / "QA-PORTFOLIO-ASSETS" / "Sprint-11"
MANIFEST = ROOT / "asset-browser" / "manifest-sprint-11.json"
OUTPUT = ROOT / "QA-Portfolio-Sprint-11-Assets.zip"
EXPECTED = [
    "95-prime-baby-gear",
    "96-ollie-burwell",
    "97-nokoluxe",
...
def validate() -> dict:
    ...
    if len(manifest.get("projects", [])) != 4:
        raise SystemExit("Sprint 11 manifest must contain exactly four projects")
    if manifest.get("totalImages", 0) < 40 or manifest.get("totalVideos", 0) < 4:
        raise SystemExit("Sprint 11 minimum counts are not met")
...
def main() -> None:
    ...
    with zipfile.ZipFile(OUTPUT, "w", zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
        for file in sorted(SPRINT.rglob("*")):
            if file.is_file():
                archive.write(file, Path("Sprint-11") / file.relative_to(SPRINT))
    with zipfile.ZipFile(OUTPUT) as archive:
        if archive.testzip():
            raise SystemExit("Sprint 11 ZIP integrity test failed")
        for folder in EXPECTED:
            if not any(name.startswith(f"Sprint-11/{folder}/") for name in archive.namelist()):
