#!/usr/bin/env python3
"""Build and integrity-check the downloadable Sprint 3 package."""
...
SPRINT = ROOT / "QA-PORTFOLIO-ASSETS" / "Sprint-03"
MANIFEST = ROOT / "asset-browser" / "manifest-sprint-03.json"
OUTPUT = ROOT / "QA-Portfolio-Sprint-03-Assets.zip"
EXPECTED = [
    "21-moore-beauty", "22-kuurth", "23-biofield-resonator", "24-xtinctio",
    "25-wendy-mccallum-art", "26-e-and-e-essentials", "27-anthom-design-house",
    "28-gabrielle-jewelry", "29-black-arrow-gallery", "30-cowbells",
]
...
def validate() -> dict:
    ...
    if len(manifest.get("projects", [])) != 10:
        raise SystemExit("Sprint 3 manifest must contain exactly ten projects")
    if manifest.get("totalImages", 0) < 100 or manifest.get("totalVideos", 0) < 10:
        raise SystemExit("Sprint 3 minimum counts are not met")
...
def main() -> None:
    ...
    with zipfile.ZipFile(OUTPUT, "w", zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
        for file in sorted(SPRINT.rglob("*")):
            if file.is_file():
                archive.write(file, Path("Sprint-03") / file.relative_to(SPRINT))
    with zipfile.ZipFile(OUTPUT) as archive:
        if archive.testzip():
            raise SystemExit("Sprint 3 ZIP integrity test failed")
        for folder in EXPECTED:
            if not any(name.startswith(f"Sprint-03/{folder}/") for name in archive.namelist()):
