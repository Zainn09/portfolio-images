#!/usr/bin/env python3
"""Build and integrity-check the downloadable Sprint 5 package."""
...
SPRINT = ROOT / "QA-PORTFOLIO-ASSETS" / "Sprint-05"
MANIFEST = ROOT / "asset-browser" / "manifest-sprint-05.json"
OUTPUT = ROOT / "QA-Portfolio-Sprint-05-Assets.zip"
EXPECTED = [
    "41-artdiscount",
    "42-cryptic-mushrooms",
    "43-divchi",
    "44-chicopick",
    "45-hugh-mcelvanna-menswear",
    "46-j-fredric",
    "47-bloomsbury-flowers",
    "48-nordikka-collective",
    "49-my-travel-passport",
    "50-peter-james-jewelry",
]
...
def validate() -> dict:
    ...
    if len(manifest.get("projects", [])) != 10:
        raise SystemExit("Sprint 5 manifest must contain exactly ten projects")
    if manifest.get("totalImages", 0) < 100 or manifest.get("totalVideos", 0) < 10:
        raise SystemExit("Sprint 5 minimum counts are not met")
...
def main() -> None:
    ...
    with zipfile.ZipFile(OUTPUT, "w", zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
        for file in sorted(SPRINT.rglob("*")):
            if file.is_file():
                archive.write(file, Path("Sprint-05") / file.relative_to(SPRINT))
    with zipfile.ZipFile(OUTPUT) as archive:
        if archive.testzip():
            raise SystemExit("Sprint 5 ZIP integrity test failed")
        for folder in EXPECTED:
            if not any(name.startswith(f"Sprint-05/{folder}/") for name in archive.namelist()):
