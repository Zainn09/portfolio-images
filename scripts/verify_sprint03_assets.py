#!/usr/bin/env python3
"""Final Sprint 3 image, playable-video, naming, manifest, and ZIP QA."""
...
MANIFEST_FILE = ROOT / "asset-browser" / "manifest-sprint-03.json"
ZIP_FILE = ROOT / "QA-Portfolio-Sprint-03-Assets.zip"
...
if not ZIP_FILE.is_file():
    errors.append("Sprint 3 ZIP is missing")
else:
    with zipfile.ZipFile(ZIP_FILE) as archive:
        if archive.testzip():
            errors.append("Sprint 3 ZIP integrity failed")
        if "Sprint-03/README.md" not in archive.namelist():
            errors.append("Sprint 3 ZIP is missing the sprint README")

if errors:
    print("SPRINT 3 FINAL QA FAILED")
    print("\n".join(f"- {error}" for error in errors))
    raise SystemExit(1)

print("SPRINT 3 FINAL QA PASSED")
