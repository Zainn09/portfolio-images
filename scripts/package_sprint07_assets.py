#!/usr/bin/env python3
"""Build and integrity-check the downloadable Sprint 7 package."""
...
SPRINT = ROOT / "QA-PORTFOLIO-ASSETS" / "Sprint-07"
MANIFEST = ROOT / "asset-browser" / "manifest-sprint-07.json"
OUTPUT = ROOT / "QA-Portfolio-Sprint-07-Assets.zip"
EXPECTED = [
    "61-rocket-krunch",
    "62-rahiza",
    "63-total-beauty-experience",
    "64-john-white-shoes",
    "65-love-renaissance",
    "66-hughies-dog-accessories",
    "67-vezorla",
    "68-op-fashion-fabrics",
    "69-gym-emotion",
    "70-elysian-blooms",
]
...
def validate() -> dict:
    ...
    if len(manifest.get("projects", [])) != 10:
        raise SystemExit("Sprint 7 manifest must contain exactly ten projects")
    if manifest.get("totalImages", 0) < 100 or manifest.get("totalVideos", 0) < 10:
        raise SystemExit("Sprint 7 minimum counts are not met")
...
def main() -> None:
    ...
    with zipfile.ZipFile(OUTPUT, "w", zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
        for file in sorted(SPRINT.rglob("*")):
            if file.is_file():
                archive.write(file, Path("Sprint-07") / file.relative_to(SPRINT))
    with zipfile.ZipFile(OUTPUT) as archive:
        if archive.testzip():
            raise SystemExit("Sprint 7 ZIP integrity test failed")
        for folder in EXPECTED:
            if not any(name.startswith(f"Sprint-07/{folder}/") for name in archive.namelist()):
