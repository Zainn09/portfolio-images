...
const ROOT = path.resolve('QA-PORTFOLIO-ASSETS/Sprint-10');
...
async function captureTmiSourceProject(project) {
  ...
  const result = {
    ...
    images: captures.map(item => ({ ...item, path: `QA-PORTFOLIO-ASSETS/Sprint-10/${project.folder}/images/${item.file}` })),
    videos: [{ ...video, path: `QA-PORTFOLIO-ASSETS/Sprint-10/${project.folder}/video/${video.file}`, thumbnailPath: `QA-PORTFOLIO-ASSETS/Sprint-10/${project.folder}/video/${video.thumbnail}` }],
    notes
  };
...
async function captureProject(browser, project) {
  ...
  try {
    ...
    const result = {
      id: project.id, slug: project.slug, folder: project.folder, name: project.name,
      url: project.url, kind: project.kind, generated: CAPTURE_DATE,
      images: captures.map(c => ({ ...c, path: `QA-PORTFOLIO-ASSETS/Sprint-10/${project.folder}/images/${c.file}` })),
      videos: [{ ...video, path: `QA-PORTFOLIO-ASSETS/Sprint-10/${project.folder}/video/${video.file}`, thumbnailPath: `QA-PORTFOLIO-ASSETS/Sprint-10/${project.folder}/video/${video.thumbnail}` }],
      notes
    };
    await fs.writeFile(path.join(base, 'asset-manifest.json'), JSON.stringify(result, null, 2));
...
async function createBrowser(manifest) {
  ...
  await fs.writeFile('asset-browser/manifest-sprint-10.json', JSON.stringify(manifest, null, 2));
}
...
async function updateRootProgress(results, stage, activeProject = null, blocked = false, blockedNote = '') {
  ...
  await fs.writeFile('capture-progress-sprint-10.json', JSON.stringify(progress, null, 2));
  ...
  await fs.mkdir('asset-browser', { recursive: true });
  await fs.writeFile('asset-browser/manifest-sprint-10.json', JSON.stringify({
    title: 'QA Portfolio Visual Assets — Sprint 10', generated: CAPTURE_DATE,
    totalImages, totalVideos, projects: results
  }, null, 2));
  await execFileAsync('python3', ['scripts/update_readme_sprint10.py']);
...
async function publishCheckpoint(project) {
  ...
  await execFileAsync('git', ['add', 'README.md', 'capture-progress-sprint-10.json', 'asset-browser/manifest-sprint-10.json', 'asset-browser/manifest.json', path.join(ROOT, project.folder)]);
  await execFileAsync('git', ['commit', '-m', `Add Sprint 10 ${project.id} ${project.name} asset checkpoint [skip ci]`]);
...
async function publishBlockedProgress(project) {
  ...
  await execFileAsync('git', ['add', 'README.md', 'capture-progress-sprint-10.json', 'asset-browser/manifest-sprint-10.json', 'asset-browser/manifest.json', path.join(ROOT, project.folder)]);
  const commit = await execFileAsync('git', ['commit', '-m', `Document Sprint 10 ${project.id} capture interruption [skip ci]`]).catch(() => null);
  if (commit) {
    await execFileAsync('git', ['fetch', 'origin', branch]);
    await execFileAsync('git', ['rebase', 'FETCH_HEAD']);
...
async function main() {
  ...
  await updateRootProgress(results, 'All Sprint 10 project assets generated; final QA pending');
  const totalImages = results.reduce((sum, p) => sum + p.images.length, 0);
  const totalVideos = results.reduce((sum, p) => sum + p.videos.length, 0);
  const readme = `# QA Portfolio Visual Asset Library — Sprint 10\n\n` +
  ...
  const manifest = { title: 'QA Portfolio Visual Assets — Sprint 10', generated: CAPTURE_DATE, totalImages, totalVideos, projects: results };
  await createBrowser(manifest);
  await fs.writeFile('capture-report-sprint-10.json', JSON.stringify(manifest, null, 2));
  await fs.rm(TMP, { recursive: true, force: true });
  console.log(`\nSprint 10 capture complete: ${totalImages} images, ${totalVideos} videos.`);
  if (results.length !== 4 || totalImages < 40 || totalVideos < 4) throw new Error('Minimum delivery count was not met');
}
