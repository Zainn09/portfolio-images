#!/usr/bin/env python3
"""Final Sprint 5 image, playable-video, naming, manifest, and ZIP QA."""
...
MANIFEST_FILE = ROOT / "asset-browser" / "manifest-sprint-05.json"
ZIP_FILE = ROOT / "QA-Portfolio-Sprint-05-Assets.zip"
...
if not ZIP_FILE.is_file():
    errors.append("Sprint 5 ZIP is missing")
else:
    with zipfile.ZipFile(ZIP_FILE) as archive:
        if archive.testzip():
            errors.append("Sprint 5 ZIP integrity failed")
        if "Sprint-05/README.md" not in archive.namelist():
            errors.append("Sprint 5 ZIP is missing the sprint README")

if errors:
    print("SPRINT 5 FINAL QA FAILED")
    print("\n".join(f"- {error}" for error in errors))
    raise SystemExit(1)

print("SPRINT 5 FINAL QA PASSED")
